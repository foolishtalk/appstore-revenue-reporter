...
工作流每天 `12:17 UTC` 运行，即北京时间 `20:17`，对应 Pacific Time 的 `04:17` 或 `05:17`。任务默认获取最近通常可用的日报日，因此即使当天日报尚未发布，也会回退到前一个可用日期。原始日报和冻结汇率使用滚动 Actions Cache；Artifact 保留 30 天。
