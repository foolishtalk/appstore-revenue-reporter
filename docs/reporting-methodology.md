...
销售项目使用 Apple 日报的 `Title` 字段（通常为 App 名称或 App 内购买项目 ID），同一周期内按项目合并 `Units`。合并后的净数量为 0 时，该项目不写入 JSON，也不显示在企业微信消息中。

退款数量单独汇总付费行中负 `Units` 的绝对值，并按 `Title` 展示退款项目。负 `Units` 仍会同时冲减净销售数量。`CMB` 字段为 `CMB-C` 的已购 App 套装抵扣不是退款，不计入退款数量；周期内没有退款时，不显示退款行。
...
销售数量的涨跌幅使用同一公式，其中 CNY 金额替换为净 `Units`；上期为 0 时同样显示“新增”“转负”或“持平”。
